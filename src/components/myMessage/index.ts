import Vue from 'vue'
import messageComponent from './index.vue' // 引入组件，方便继承
const MessageConstructor = Vue.extend(messageComponent) // 引入一个message构造器，方便new之
...
const MyMessage = function (options: Options) {
  if (options.duration && typeof options.duration !== 'number') {
    // 对于duration数字类型的校验
    return console.error('Error! duration Must be a numeric type ') // 用户乱传递非数字类型参数，就抛错不执行后续代码
  }
...
function cutCount() {
  ...
  const messageBoxDomList = document.querySelectorAll('.messageBox') as NodeListOf<HTMLElement> // 然后选中所有的messageDOM元素
  for (let i = 0; i < messageBoxDomList.length; i++) {
    // 遍历一下这个DOM伪数组
    const dom = messageBoxDomList[i] // 所有的都往上移动60像素
